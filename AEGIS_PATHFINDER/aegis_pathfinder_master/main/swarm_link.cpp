...
static const swarm_node_id_t SELF_NODE_ID = SWARM_NODE_PATHFINDER;
 ...
 * the Wi-Fi driver task's own context. Found on Guardian (same pattern
 * there) that calling anything non-trivial from there can stall other
 * core-0 work badly enough to starve unrelated tasks. Record the pending
 * state here instead; heartbeat_timer_cb() below is the only place that
 * actually invokes these callbacks. */
 ...
 * already exists under a different one. Pathfinder's own init_master_espnow()
 * already registers the FF:FF:FF:FF:FF:FF broadcast peer at its Master<->Slave
 * channel (9) before this runs — a blind esp_now_add_peer() here would fail
 * with ESP_ERR_ESPNOW_EXIST, so this retunes that existing entry to channel 6
 * instead of adding a second one (ESP-NOW keys peers by MAC address only). */
...
esp_err_t swarm_send_map_update(float x, float y, bool has_door, uint8_t snap_index, uint8_t total_snaps)
{
    ...
    payload.x = x;
    payload.y = y;
    payload.has_door = has_door;
    payload.snap_index = snap_index;
    payload.total_snaps = total_snaps;

    return send_packet(SWARM_MSG_MAP_UPDATE, &payload, sizeof(payload));
}
...
esp_err_t swarm_link_init(void)
{
    ...
    if ((err = ensure_peer(s_broadcast_mac, AEGIS_SWARM_CHANNEL)) != ESP_OK) {
        ESP_LOGE(TAG, "retune broadcast peer failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    ESP_LOGI(TAG, "Swarm link ready on channel %d (Pathfinder)", AEGIS_SWARM_CHANNEL);
    return ESP_OK;
}
