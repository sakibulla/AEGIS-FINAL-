...
/* Scans all nodes (Guardian included) for the first active alert and updates
 * the big banner above the status strip. This — not the small 80x40 cells —
 * is meant to be the thing you actually notice from across a room. */
...
{
    int idx = (int)SELF_NODE_ID;
    if (s_cell[idx] == nullptr) {
        return;
    }
    ...
    bool has_alert = (alert_text != nullptr && alert_text[0] != '\0');
    s_node_has_alert[idx] = has_alert;
    if (has_alert) {
        snprintf(s_node_alert_text[idx], sizeof(s_node_alert_text[idx]), "%s", alert_text);
    } else {
    ...
    lv_color_t bg = has_alert ? lv_palette_main(LV_PALETTE_RED) : lv_palette_main(LV_PALETTE_BLUE);
    const char *status = has_alert ? alert_text : "(self)";
    lv_obj_set_style_bg_color(s_cell[idx], bg, 0);
    lv_label_set_text_fmt(s_label[idx], "%s\n%s", NODE_NAME[idx], status);
